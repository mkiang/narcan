        ...
        new_df <- processed_df %>%
            mutate(
                ...
                heroin_present = case_when(
                    grepl(ucod, pattern = "E8500") ~ 1,
                    grepl(f_records_all, pattern = "E8500") ~ 1,
                    TRUE ~ 0),
                other_natural_present = 0,
                methadone_present = case_when(
                    grepl(ucod, pattern = "E8501") ~ 1,
                    grepl(f_records_all, pattern = "E8501") ~ 1,
                    TRUE ~ 0),
                other_synth_present = 0,
                other_op_present = case_when(
                    grepl(ucod, pattern = "E8502") ~ 1,
                    grepl(f_records_all, pattern = "E8502") ~ 1,
                    TRUE ~ 0))
    } else {
        new_df <- processed_df %>%
            mutate(
                opium_present = case_when(
                    grepl(f_records_all, pattern = "T400") ~ 1, TRUE ~ 0),
                heroin_present = case_when(
                    grepl(f_records_all, pattern = "T401") ~ 1, TRUE ~ 0),
                other_natural_present = case_when(
                    grepl(f_records_all, pattern = "T402") ~ 1, TRUE ~ 0),
                methadone_present = case_when(
                    grepl(f_records_all, pattern = "T403") ~ 1, TRUE ~ 0),
                other_synth_present = case_when(
                    grepl(f_records_all, pattern = "T404") ~ 1, TRUE ~ 0),
                other_op_present = case_when(
                    grepl(f_records_all, pattern = "T406") ~ 1, TRUE ~ 0))
    }
