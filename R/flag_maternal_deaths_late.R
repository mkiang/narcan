...
flag_maternal_deaths_late <- function (processed_df, year = NULL,
    ...
    df <- processed_df |>
        dplyr::mutate(maternal_death_late = (
            grepl(.regex_maternal_icd10(include_late = TRUE), ucod)) + 0)
    ...
    if (ucod_only == FALSE) {
        ...
        df <- df |>
            dplyr::mutate(maternal_death_late = dplyr::case_when(
    ...
    if (!keep_cols) {
        df <- suppressMessages(suppressWarnings(
            dplyr::select(
                df, dplyr::any_of(c(original_cols, "maternal_death_late"))
            )
        ))
    }
